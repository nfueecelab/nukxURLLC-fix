...
#if(NUK)
// NUK functions
void enb::set_split_ratio(uint8_t ratio_MeNB_, uint8_t ratio_SeNB_)
{
  stack->set_split_ratio(ratio_MeNB_, ratio_SeNB_);
}

void enb::set_split_mode()
{
  stack->set_split_mode();
}

void enb::set_lossrate(uint8_t loss_MeNB_, uint8_t loss_SeNB_)
{
  stack->set_lossrate(loss_MeNB_, loss_SeNB_);
}

void enb::set_duplication_mode()
{
  stack->set_duplication_mode();
}
#endif
