...
int enb_stack_lte::init(const stack_args_t& args_, const rrc_cfg_t& rrc_cfg_)
{
  ...
  #if(NUK)
  rlc.init(&pdcp, &rrc, &mac, &timers, &rlc_log, args.x2ap_myaddr, args.x2ap_neiaddr);
  #else
...
#if(NUK)
void enb_stack_lte::set_split_ratio(uint8_t ratio_MeNB, uint8_t ratio_SeNB)
{
  rlc.set_split_ratio(ratio_MeNB,ratio_SeNB);
}
void enb_stack_lte::set_split_mode()
{
  rlc.set_split_mode();
}
void enb_stack_lte::set_lossrate(uint8_t loss_MeNB, uint8_t loss_SeNB)
{
  rlc.set_lossrate(loss_MeNB, loss_SeNB);
}
void enb_stack_lte::set_duplication_mode()
{
  rlc.set_duplication_mode();
}
#endif
