...
namespace srslte {
#if(NUK && NUK_UE)
class pdcp : public srsue::pdcp_interface_rlc, public srsue::pdcp_interface_rrc, public thread
{
public:
  ...
  void init(srsue::rlc_interface_pdcp* rlc_, srsue::rrc_interface_pdcp* rrc_, srsue::gw_interface_pdcp* gw_, std::string node_type_);
...
// NUK private member
  int socket_fd = -1;
  int node_status = -1; 
  void create_inter_socket(int node_status_);
  void set_socket();
  struct sockaddr_un ser,cli;
  const char* const ser_name = "server";
  const char* const cli_name = "client";
  char const *ifname = "srsue1";
  srslte::byte_buffer_pool* pool;
  uint32_t cp_lcid;
  void local_communication(unique_byte_buffer_t pdu);
  static const int THREAD_PRIO = 65;
  bool thread_running = false;
  bool thread_run_enable = false;
  void run_thread();
#if(NUK_JIN_DEBUG)
  int ue2_count = 0;
  int ue1_count = 0;
